...
/**
 * An adapter that tries to put sane defaults for default return values. Will
 * silently pretend that most non-critical operations have succeeded, but return
 * ENOSYS on non-implemented important operations.
 */
public abstract class FuseFilesystemAdapterFull extends FuseFilesystem {
	...
	@Override
	public int fgetattr(final String path, final StatWrapper stat,
			final FileInfoWrapper info) {
	...
	@Override
	public int fsync(final String path, final int datasync,
			final FileInfoWrapper info) {
		return 0;
	}

	@Override
	public int fsyncdir(final String path, final int datasync,
			final FileInfoWrapper info) {
		return 0;
	}

	@Override
	public int ftruncate(final String path, final long offset,
			final FileInfoWrapper info) {
	...
	@Override
	public int getxattr(final String path, final String xattr,
			final XattrFiller filler, final long size, final long position) {
		return -ErrorCodes.ENOSYS();
	}
	...
	@Override
	public int setxattr(final String path, final String xattr,
			final ByteBuffer buf, final long size, final int flags,
			final int position) {
		return -ErrorCodes.ENOSYS();
	}
